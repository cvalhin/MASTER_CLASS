...
# load TPM RESULTS: 
load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/carolina/MASTER_CLASS/lessons/05_RNAseq_in_R_studio/Results/TPM_results/TPM_results.Rdata")

# laod DESEQ2 results
load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/carolina/MASTER_CLASS/lessons/05_RNAseq_in_R_studio/Results/counts_results_DESEQ/DESEQ_results.rdata")
...
hist(average_and_stddev_values$'96_avg', xlim = c(1,1000), ylim = c(1,1000), breaks = 1000)
...
#Choose a gene of interest, I chose this one for quickness, I have to look at other candidates present in the sig_gene_tpm
gene_up <- as.data.frame(c("Napsa"))

names(gene_up) <- "gene_name"

# indexing to just get values for this gene
	
Napsa_tpm <- sig_gene_tpm[sig_gene_tpm$gene_name %in% gene_up$gene_name, ]

# now moving the character gene_name to rownames to won't interfere with numeric values
rownames(Napsa_tpm) <- Napsa_tpm$gene_name
...
avg_cols <- grep("_avg", names(Napsa_tpm), value = TRUE)

sd_cols <- grep("_sd", names(Napsa_tpm), value = TRUE)

# This is another way to "melt" or pivot longer
plot_df <- data.frame(
  time = as.numeric(gsub("_avg", "", avg_cols)),
  avg = unlist(Napsa_tpm[avg_cols]),
  sd = unlist(Napsa_tpm[sd_cols])
)
...
# Convert Time to numeric values to be able to plot
